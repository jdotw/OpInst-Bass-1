...
void print_csv_time(void) {
  ...
  PRINT_CSV("%02ld:%02ld:%02ld.%03ld", (long)(ms / (60 * 60 * 1000) % 24),
            (long)(ms / (60 * 1000) % 60), (long)((ms / 1000) % 60),
            (long)(ms % 1000));
}
#endif
...
static void sendData(uint8_t *data_buffer, uint8_t Nb_bytes) {
  ...
  if (device_role == SLAVE_ROLE) {
    while (aci_gatt_update_char_value_ext(
               connection_handle, sampleServHandle, TXCharHandle, 1, Nb_bytes,
               0, Nb_bytes, data_buffer) == BLE_STATUS_INSUFFICIENT_RESOURCES) {
    ...
    while (aci_gatt_write_without_resp(connection_handle, rx_handle + 1,
                                       Nb_bytes,
                                       data_buffer) == BLE_STATUS_NOT_ALLOWED) {
      hci_user_evt_proc();
      // Radio is busy (buffer full).
      if ((HAL_GetTick() - tickstart) > (10 * HCI_DEFAULT_TIMEOUT_MS))
        break;
 ...
 * Input          : Data length.
 *                  Data value
 * Return         : TRUE if the local name found is the expected one, FALSE
 *otherwise.
 *******************************************************************************/
static uint8_t Find_DeviceName(uint8_t data_length, uint8_t *data_value) {
  uint8_t index = 0;
  ...
  while (index < data_length) {
    ...
    if (data_value[index + 1] == AD_TYPE_COMPLETE_LOCAL_NAME) {
      ...
      if (BLUENRG_memcmp(&data_value[index + 1], &local_name[0],
                         BLE_SAMPLE_APP_COMPLETE_LOCAL_NAME_SIZE) == 0) {
        return TRUE;
      } else {
        return FALSE;
      }
...
static void Connection_StateMachine(void) {
  ...
  case (START_DISCOVERY_PROC): {
    ...
    ret =
        aci_gap_start_general_discovery_proc(SCAN_P, SCAN_L, PUBLIC_ADDR, 0x00);
    if (ret != BLE_STATUS_SUCCESS) {
  ...
  case (DO_DIRECT_CONNECTION_PROC): {
    ...
    ret = aci_gap_create_connection(
        SCAN_P, SCAN_L, discovery.device_found_address_type,
  ...
  case (ENTER_DISCOVERY_MODE): {
    ...
    ret = aci_gap_set_discoverable(
        ADV_DATA_TYPE, ADV_INTERV_MIN, ADV_INTERV_MAX, PUBLIC_ADDR,
        NO_WHITE_LIST_USE, sizeof(local_name), local_name, 0, NULL, 0x0, 0x0);
    if (ret != BLE_STATUS_SUCCESS) {
  ...
  if (device_role == MASTER_ROLE) {
    ...
    if (APP_FLAG(CONNECTED) && !APP_FLAG(END_READ_TX_CHAR_HANDLE)) {
      if (!APP_FLAG(START_READ_TX_CHAR_HANDLE)) {
        ...
        aci_gatt_disc_char_by_uuid(connection_handle, 0x0001, 0xFFFF,
    ...
    if (APP_FLAG(CONNECTED) && APP_FLAG(END_READ_TX_CHAR_HANDLE) &&
        APP_FLAG(END_READ_RX_CHAR_HANDLE) && !APP_FLAG(NOTIFICATIONS_ENABLED)) {
      ...
      if ((mtu_exchanged == 0) && (mtu_exchanged_wait == 0)) {
        ...
        while (aci_gatt_write_char_desc(connection_handle, tx_handle + 2, 2,
  ...
  if (BSP_PB_GetState(BUTTON_KEY) == !user_button_init_state) {
    ...
    if (APP_FLAG(CONNECTED) && APP_FLAG(NOTIFICATIONS_ENABLED)) {
      ...
      while (data_ptr < (data + sizeof(data))) {
        /* if data to send are greater than the max char value length, send them
         * in chunks */
        curr_len = ((data + sizeof(data) - data_ptr) > write_char_len)
                       ? (write_char_len)
                       : (data + sizeof(data) - data_ptr);
...
void hci_le_connection_complete_event(
    uint8_t Status, uint16_t Connection_Handle, uint8_t Role,
    uint8_t Peer_Address_Type, uint8_t Peer_Address[6], uint16_t Conn_Interval,
    uint16_t Conn_Latency, uint16_t Supervision_Timeout,
    uint8_t Master_Clock_Accuracy)

{
  /* Set the exit state for the Connection state machine:
...
void aci_gatt_disc_read_char_by_uuid_resp_event(uint16_t Connection_Handle,
  ...
  if (APP_FLAG(START_READ_TX_CHAR_HANDLE) &&
    ...
    if (APP_FLAG(START_READ_RX_CHAR_HANDLE) &&
        !APP_FLAG(END_READ_RX_CHAR_HANDLE)) {
