...
static const ctrl_toggle_t _init_ctrl_toggle = {0};
...
void ctrl_toggle_init() { ctrl_toggle = _init_ctrl_toggle; }
...
void ctrl_apply_toggle(enc_enum_t ctrl, bool changed, bool state) {
  ...
  if (ctrl == ENC_FX_WETDRY) {
    if (state && changed) {
      ctrl_toggle.fx_wetdry_func = ENC_FX_WETDRY_PRESET;
    } else if (changed) {
      ctrl_toggle.fx_wetdry_func = ENC_FX_WETDRY_FX;
    }
  }
}
