...
void midi_rx_request_bytes(uint8_t count, uint8_t offset) {
	HAL_StatusTypeDef res = HAL_UART_Receive_DMA(midi_rx_uart, (uint8_t *)rx_buffer + offset, count);
//	HAL_StatusTypeDef res = HAL_UART_Receive_IT(midi_rx_uart, (uint8_t *)rx_buffer + offset, count);
	if (res != HAL_OK) {
		Error_Handler();
	}
