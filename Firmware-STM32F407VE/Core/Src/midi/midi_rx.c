...
uint8_t handle_midi_rx_data(uint8_t status_msg, uint8_t *data,
                            uint8_t recv_count, uint8_t offset);
...
void midi_rx_set_uart(UART_HandleTypeDef *huart) { midi_rx_uart = huart; }
...
void midi_rx_request_bytes(uint8_t count, uint8_t offset) {
  ...
  //	HAL_StatusTypeDef res = HAL_UART_Receive_IT(midi_rx_uart, (uint8_t
  //*)rx_buffer + offset, count);
  if (res != HAL_OK) {
    Error_Handler();
  }
  //  HAL_UART_Receive_IT(midi_rx_uart, (uint8_t *)rx_buffer + offset, count);
  rx_recv_count = count;
}
...
void midi_rx_dma_rxcomplete_callback() {
  if ((rx_buffer[0] & MIDI_STATUS_MASK) == MIDI_STATUS_MSG) {
    ...
    active_status_msg = handle_midi_rx_data(active_status_msg, rx_buffer,
                                            rx_recv_count, rx_recv_offset);
  }
...
uint8_t handle_midi_rx_data(uint8_t status_msg, uint8_t *data,
  ...
  uint8_t expected_data_byte_count =
      midi_data_bytes_count_for_status(status_msg);
  if (actual_data_byte_count == expected_data_byte_count) {
    ...
    midi_rx_request_bytes(expected_data_byte_count - actual_data_byte_count,
                          actual_data_byte_count);
    return status_msg;
  } else {
