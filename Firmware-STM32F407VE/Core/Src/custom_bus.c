...
int32_t BSP_SPI1_Send(uint8_t *pData, uint16_t Length) {
  ...
  if (HAL_SPI_Transmit(&hspi1, pData, Length, BUS_SPI1_POLL_TIMEOUT) !=
      HAL_OK) {
...
int32_t BSP_SPI1_RegisterDefaultMspCallbacks(void) {
  ...
  if (HAL_SPI_RegisterCallback(&hspi1, HAL_SPI_MSPINIT_CB_ID, SPI1_MspInit) !=
      HAL_OK) {
    return BSP_ERROR_PERIPH_FAILURE;
  }

  /* Register MspDeInit Callback */
  if (HAL_SPI_RegisterCallback(&hspi1, HAL_SPI_MSPDEINIT_CB_ID,
                               SPI1_MspDeInit) != HAL_OK) {
    return BSP_ERROR_PERIPH_FAILURE;
  }
  IsSPI1MspCbValid = 1;
...
int32_t BSP_SPI1_RegisterMspCallbacks(BSP_SPI_Cb_t *Callbacks) {
  ...
  /* Register MspInit Callback */
  if (HAL_SPI_RegisterCallback(&hspi1, HAL_SPI_MSPINIT_CB_ID,
 ...
 */
int32_t BSP_GetTick(void) { return HAL_GetTick(); }
