...
uint16_t *_button_step_rgb(uint8_t i) {
  ...
  if (button.pressed) {
    ...
    bool isActive =
        i == commit_seq_state.active_step % 16 && commit_seq_state.running;
    val[0] = MIN_PWM;
...
bool _commit_led_steps1to12_changed() {
  ...
  if (commit_seq_changed.active_step || commit_seq_changed.running) {
    return true;
  }
  return false;
}
...
bool _commit_led_shiftpage_changed() {
  return commit_mod_state.button_changed.shift ||
         ...
         commit_seq_changed.active_page || commit_seq_changed.running ||
         blink_changed;
}
...
void _commit_led_button_steps1to12() {
  ...
  if (!_commit_led_steps1to12_changed())
    return;
...
void _commit_led_button_steps13to16() {
  ...
  if (!_commit_led_steps13to16_changed())
    return;
...
void _commit_led_button_mod_shiftpage() {
  ...
  if (!_commit_led_shiftpage_changed())
    return;

  // Shift is 0,1,2 [actually 24,25,26]
  _set_pwm_single(pwm_seq,
                  _button_shift_rgb(commit_mod_state.button_state.shift));
  _set_button_scale_seq(pwm_seq, scale_seq, 4 * 3);
...
void _commit_led_button_mod_start() {
  ...
  if (!_commit_led_start_changed())
    return;

  _set_pwm_single(pwm_seq,
                  _button_start_rgb(commit_mod_state.button_state.start));
