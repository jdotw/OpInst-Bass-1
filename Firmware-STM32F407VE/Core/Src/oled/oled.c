...
void _oled_flush_cb(struct _lv_disp_drv_t *disp_drv, const lv_area_t *area,
                    lv_color_t *color_p) {
...
void oled_test() {
  ...
  //  lv_label_set_long_mode(label1, LV_LABEL_LONG_WRAP);     /*Break the long
  //  lines*/ lv_label_set_recolor(label1, true);                      /*Enable
  //  re-coloring by commands in the text*/ lv_label_set_text(label1, "#0000ff
  //  Re-color# #ff00ff words# #ff0000 of a# label, align the lines to the
  //  center "
  //      "and wrap long text automatically.");
  //  lv_obj_set_width(label1, 150);  /*Set smaller width to make the lines
  //  wrap*/ lv_obj_set_style_text_align(label1, LV_TEXT_ALIGN_CENTER, 0);
